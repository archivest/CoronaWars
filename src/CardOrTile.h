...
class CardOrTile : public DragableTokenContainer
{
    ...
    SimpleImage m_image;
    CoordinateMappingRotateAndColour m_mapping;
...
public:
    ...
    void updateTransparency()
    {
        if (m_iTransparencyX > -1 && m_iTransparencyY > -1)
        {
            ...
            {
                unsigned int uiTransparencyColour = m_image.getRawImageData()->getRawPixelColour(m_iTransparencyX, m_iTransparencyY);
                m_mapping.setTransparencyColour( m_image.getRawImageData()->getRawPixelColour(m_iTransparencyX, m_iTransparencyY) );
#if AVOID_BLIT == 0
                // As long as not avoiding a blit then we need to set the transparency amount of the pixels in case of doign a blit...
                m_image.getRawImageData()->setTransparentColour(uiTransparencyColour);
#endif
            }
        ...
        else
        {
            if (m_image.getRawImageData() != nullptr)
            {
                m_mapping.setTransparencyColour( m_iDefaultTransparencyColour );

#if AVOID_BLIT == 0
                // As long as not avoiding a blit then we need to set the transparency amount of the pixels in case of doign a blit...
                m_image.getRawImageData()->setTransparentColour(m_iDefaultTransparencyColour);
#endif
            }
        }
    }
    ...
    virtual void virtDraw() override
    {
        if (isVisible())
        {
            ...
            {    
                ...
                // Draw image
                m_image.renderImageBlit( getEngine(), getEngine()->getForegroundSurface(),
                    m_iCurrentScreenX + m_iStartDrawPosX, m_iCurrentScreenY + m_iStartDrawPosY, 
                    m_iWidthInImage / m_iCurrentScalingLevel, m_iHeightInImage / m_iCurrentScalingLevel,
                    m_iLeftInImage, m_iTopInImage, 
                    m_iWidthInImage, m_iHeightInImage);
            }
