...
/*
This supports a class which can map coordinates of an image when drawing it
See the ImageMappingDemo for an example of its use
It can be used for things like rotating or shifting images.
Important note: It could be SLOW! It applies the transformation to each pixel. If the tranformation is complex (e.g. rotation) this could take it a while!
*/
...
/*
This is a coordinate mapping function whose only purpose is to avoid drawing outside of the viewable area
*/
class CroppedCoordinateMapping : public CoordinateMapping
{
public:
...
/* 
This is a coordiante mapping function for shifting x and y coordinates, e.g. to scroll around
*/
class CoordinateMappingShiftXY : public CoordinateMapping
{
public:
    CoordinateMappingShiftXY(double x, double y)
        : xShift(x), yShift(y)
    {
    }
...
*/
class CoordinateMappingRotate : public CoordinateMapping
{
public:
    CoordinateMappingRotate(double r)
        : rotation(r)
    {
    }
...
/* Class to do with coordiante mapping and colouring if desired */
class CoordinateMappingRotateAndColour : public CoordinateMappingRotate
{
public:
    // Rotation and rotation centre are set in constructor
    CoordinateMappingRotateAndColour(double r , int iRotationCentreX, int iRotationCentreY )
        : CoordinateMappingRotate(r), m_iRotationCentreX(iRotationCentreX), m_iRotationCentreY(iRotationCentreY),
    ...
    // No rotation stored, so everything gets initialised - remember to set the values you need!
    CoordinateMappingRotateAndColour()
        : CoordinateMappingRotate(0), m_iRotationCentreX(0), m_iRotationCentreY(0),
        m_iTransparencyColour(-1),
        m_iEverySecondPixelColour(-1), m_iEveryThirdPixelColour(-1), m_iEveryFourthPixelColour(-1),
        m_iMergeColour(-1), m_iMergePercentage(-1), m_iBrightnessPercentage(100),
        m_iSourceShiftX(0), m_iSourceShiftY(0)
    ...
    virtual ~CoordinateMappingRotateAndColour()
    {
    }
    ...
    // Think of this as doing the same as the CoordinateMappingShiftXY objects
