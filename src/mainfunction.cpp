...
int doProgram(int argc, char* argv[])
{
    ...
    EzyjddEngineCW4 oMain;
    char buf[1024];
    sprintf(buf, "EZYJDD C++ Coursework 4 Framework Program : Size %d x %d", BaseScreenWidth, BaseScreenHeight);
    int iResult = oMain.initialise(buf, BaseScreenWidth, BaseScreenHeight, "Cornerstone Regular.ttf", 36);
    if (iResult == 0)
    {
